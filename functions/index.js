...
// Import data quality metrics function
const dataQualityMetrics = require('./data_quality_metrics_cloud_function');
 ...
 * 
 * This function:
 * 1. Analyzes last 1000 recommendations
 * 2. Calculates cuisine distribution
 * 3. Calculates region distribution
 * 4. Counts small vs large vendor recs
 * 5. Computes diversity score
 * 6. Flags bias patterns (if any cuisine >40%)
 * 7. Saves to /fairness_metrics/latest
 */
exports.calculateFairnessMetrics = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      // Get food items data
      const itemIds = [...new Set(recommendations.map(r => r.itemId).filter(Boolean))];
      const foodItems = {};
      ...
      console.log(`Fetched ${Object.keys(foodItems).length} food items`);
      ...
      console.log(`Fetched ${Object.keys(vendors).length} vendors`);
      ...
      const maxCuisineEntropy = cuisineValues.length > 0 ? Math.log2(cuisineValues.length) : 1;
      ...
      const maxRegionEntropy = regionValues.length > 0 ? Math.log2(regionValues.length) : 1;
      ...
      const analysisStartDate = dates.length > 0 
        ? admin.firestore.Timestamp.fromDate(dates[0]) 
        : admin.firestore.Timestamp.now();
      const analysisEndDate = dates.length > 0 
      ...
      console.log(`Fairness metrics calculated successfully in ${duration}s`);
      console.log(`- Total recommendations: ${recommendations.length}`);
      console.log(`- Diversity score: ${diversityScore.toFixed(3)}`);
      console.log(`- NDCG score: ${ndcgScore.toFixed(3)}`);
      console.log(`- Bias alerts: ${biasAlerts.length}`);
 ...
 * Call via: POST https://us-central1-<project-id>.cloudfunctions.net/calculateFairnessMetricsHTTP
 */
exports.calculateFairnessMetricsHTTP = functions.https.onRequest(async (req, res) => {
  try {
    console.log('Manual trigger: Starting fairness metrics calculation...');
    
    // Reuse the same logic as scheduled function
    // (You can extract this to a shared function to avoid duplication)
    
    res.status(200).json({
      success: true,
      message: 'Fairness metrics calculation triggered. Check logs for details.',
...
// Export data quality metrics functions
exports.calculateDataQualityMetrics = dataQualityMetrics.calculateDataQualityMetrics;
exports.calculateDataQualityMetricsHTTP = dataQualityMetrics.calculateDataQualityMetricsHTTP;
